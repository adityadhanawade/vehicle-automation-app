            ...
            auth.createUserWithEmailAndPassword(email.text.toString(), password.text.toString())
                .addOnCompleteListener {
                    if(it.isSuccessful)
                    {
                        ...
                        sharedata(email.text.toString(),name.text.toString())
...
//                        Toast.makeText(applicationContext,adhar.text.toString(), Toast.LENGTH_LONG).show()
 ...
 //                       val intent = Intent(applicationContext,adminlogin::class.java)
  //                      startActivity(intent)
    ...
    private fun sharedata(email: String, name: String) {

        val sharedPreferences: SharedPreferences = this.getSharedPreferences("My",Context.MODE_PRIVATE)

...
class location : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btndeatils.setOnClickListener {
            val intent = Intent(applicationContext,Show::class.java)
        ...
        task.addOnSuccessListener { location ->
            if (location != null) {
                ...
                supportMapFragment!!.getMapAsync { googleMap ->
                    map = googleMap
                    mfire = FirebaseDatabase.getInstance().reference.child("service")
...
//                                    Toast.makeText(applicationContext,lokasi.toString(),Toast.LENGTH_LONG).show()
                                    map!!.addMarker(
                                        MarkerOptions().position(lokasi).title(s.child("garegename").value.toString())
